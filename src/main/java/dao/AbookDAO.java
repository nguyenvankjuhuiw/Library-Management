...
public class AbookDAO extends Connect {
    ...
    public List<Abook> getAbookByBookId(int bookid) throws SQLException {
        List<Abook> list = new ArrayList<>();
        String sql = "SELECT * FROM `abook` WHERE bookid = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, bookid);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
    ...
    public int getCountAbookByBookid(int bookid) throws SQLException {
        String sql = "SELECT count(*) FROM librarymanagement.abook where bookid = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, bookid);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
